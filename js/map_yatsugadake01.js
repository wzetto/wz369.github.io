...
var gpxFile = 'yatsugadake01.gpx';
function renderReport(gpxStr) {
new L.GPX(gpxFile, {
  async: true,
  marker_options: {
    startIconUrl: false,
...
var elements = window.elements = gpx.getElementsByTagName('trkpt');
var elementsAlt = gpx.getElementsByTagName('ele');
var arrEle = [];
for (var i=0; elementsAlt.length>i; i++){
...
// Check availability before initializing the map/chart and shared enhancements.
var trackCheck = new XMLHttpRequest();
trackCheck.open('get', gpxFile, false);
try {
    trackCheck.send(null);
} catch (error) {
    console.warn('Could not load the Yatsugadake track.', error);
}
if (trackCheck.status >= 200 && trackCheck.status < 300 &&
        new DOMParser().parseFromString(trackCheck.responseText, 'text/xml')
            .getElementsByTagName('trkpt').length > 0) {
    renderReport(trackCheck.responseText);
} else {
    map.fitWorld();
    document.getElementById('chart').textContent = 'GPXデータを読み込めません。';
}
